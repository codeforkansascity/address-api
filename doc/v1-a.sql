...
CREATE TABLE orgranizations ( 
	id                   integer  NOT NULL,
	name                 varchar(100)  ,
	url                  varchar(255)  ,
	CONSTRAINT pk_orgranizations PRIMARY KEY ( id )
 );

COMMENT ON COLUMN orgranizations.url IS 'URL for organization, not their data repository';
...
CREATE TABLE source_file_types ( 
	...
	description          text  ,
	created              timestamp  ,
	CONSTRAINT pk_dataset_source_types PRIMARY KEY ( id )
 );
...
CREATE TABLE source_file ( 
	...
	CONSTRAINT fk_datasets_orgranizations FOREIGN KEY ( organization_id ) REFERENCES orgranizations( id )    ,
	CONSTRAINT fk_source_file FOREIGN KEY ( source_file_type_id ) REFERENCES source_file_types( id )    
 );

CREATE INDEX idx_datasets_1 ON source_file ( organization_id );

CREATE INDEX idx_datasets_2 ON source_file ( source_file_type_id );
...
CREATE TABLE datas ( 
	...
	organization_id      integer  ,
	dataset_source_type_id integer  ,
...
CREATE INDEX idx_datasets ON datas ( dataset_source_type_id );

CREATE INDEX idx_datasets_0 ON datas ( organization_id );
...
COMMENT ON COLUMN datas.name IS 'Source name of the dataset';
...
CREATE TABLE source_fields ( 
	...
	column_no            integer  ,
	CONSTRAINT pk_dataset_fields PRIMARY KEY ( id ),
...
CREATE INDEX idx_dataset_fields ON source_fields ( data_id );

CREATE INDEX idx_dataset_fields_0 ON source_fields ( attribute_type_id );

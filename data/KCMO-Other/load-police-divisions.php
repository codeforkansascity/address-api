...
class KCMOTIF extends \Code4KC\Address\AreaLoad
{
    ...
    var $sql = "SELECT ogc_fid AS fid, wkb_geometry::geography::geometry AS geom, divisionname AS name, lastupdate, shape_length, shape_area  FROM public.policedivisions;";
        ...
        if ( $query = $address->get_all_address_lat_lng() ) {
            ...
            while ($rec = $query->fetch(PDO::FETCH_ASSOC)) {
                ...
                if ($cc_rec) {       // We found a shape this address is in
                    ...
                    $cc_rec['police_division'] = $cc_rec['name'];                                       // rename to source table field
                    unset($cc_rec['name']);

                    $new_rec = array('police_division' => $cc_rec['police_division']);
